...
def test_exit_price_per_share():
    ...
    assert abs(s.price_per_share_exit - s.equity_value_exit / 50.0) < 1e-9
...
@pytest.mark.parametrize("ticker,cik,name,wacc,net_debt,shares_m", [
    ("LULU", "0001397187", "lululemon", 0.10,  -1_000.0,  125.0),
    ("F",    "0000037996", "Ford Motor", 0.12, 20_000.0, 4_000.0),
    ("VZ",   "0000732712", "Verizon",    0.09, 120_000.0, 4_200.0),
])
...
def test_integration_full_pipeline_identities():
    ...
    inputs = DCFInputs(wacc=0.10, terminal_growth_rate=0.025,
                       exit_ebitda_multiple=10.0, net_debt=-1_000.0,
    ...
    for scen_forecast, scen_dcf in [
        ...
        assert abs(scen_dcf.price_per_share_gg -
                   scen_dcf.equity_value_gg / inputs.diluted_shares_m) < 1e-9
